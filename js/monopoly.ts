...
class Dice extends Array<Die> {
    ...
    display() {
        ...
        this.peek().map(d => {
            ...
            die.style.paddingTop = `${Math.random() * .5}em`
...
class WrapIter {
    ...
    next(currVal = this.#curr): number {
        return this.#curr = this.#max <= currVal
            ? this.#min
            : currVal + 1
    }

    prev(currVal = this.#curr): number {
        return this.#curr = currVal <= this.#min
            ? this.#max
            : currVal - 1
    }
...
class Player extends Inventory {
    ...
    get propertyTile() {
        return PROPS.filter(p =>
            p.position === this.position)[0]
    }

    doDebt() {
        loadPrompt(PROMPTS[
            this.money < GAME.currPlayer!.debt[1]!
                ? "debtCantAfford"
                : "debtCanAfford"
        ])
    }
    ...
    doUnowned() {
        loadPrompt(PROMPTS[
            this.propertyTile.value <= this.money
                ? "unownedCanAfford"
                : "unownedCantAfford"
        ])
    }
    ...
    draw(deckName: "Chance" | "Community Chest") {
    ...
    doStartTurn() {
        this.loadInventory()
        loadPrompt(PROMPTS[
            !this.inJail ? "mainPhase"
            : 50 < this.money ? "bailCanAfford"
            : "bailCantAfford"
        ])
    }
    ...
    doPass() {
        ...
        GAME.currPlayer = GAME.turnOrder[
            p.next(GAME.turnOrder.indexOf(this))]
...
const PROMPTS = Object.fromEntries(Object.entries({
    ...
    debtCanAfford: ["pay", "manage"], // chance cards bankrupt if no money?
    debtCantAfford: ["bankrupt", "manage"],
    bankrupt: ["bankrupt"],
    bailCanAfford: ["pay", "roll", "manage"],
    bailCantAfford: ["roll", "manage"],
    mustPayBail: ["pay"], // can you manage properties or do you just go bankrupt?
    manage: ["trade", "back"],
    reset: <string[]>[],
}))
